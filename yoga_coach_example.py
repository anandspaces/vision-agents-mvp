...
async def create_agent(**kwargs) -> Agent:
    agent = Agent(
        ...
        agent_user=User(name="AI Yoga Coach"),
        instructions="Read @yoga_coach.md",  # read the yoga coach markdown instructions
...
async def join_call(agent: Agent, call_type: str, call_id: str, **kwargs) -> None:
    ...
    with await agent.join(call):
        # all LLMs support a simple_response method and a more advanced native method
        await agent.llm.simple_response(
            text="Say hi. After the user begin their flow or hold a pose offer a helpful feedback."
        )
